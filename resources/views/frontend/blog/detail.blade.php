        ...
        <div class="uk-container">
            ...
            <div class="uk-child-width-1-1@s uk-child-width-1-2@m uk-child-width-2-3@l uk-text-justify uk-grid" uk-grid uk-lightbox="animation: scale">
                <article class="uk-article">
                    ...
                    <img class="uk-margin-large-top" src="{{asset('storage/'.$blogdetail->image_blog)}}" alt="">

                    <h1 class="uk-article-title uk-text-bolder"><span class="uk-link-reset">{{$blogdetail->judul_blog}}</span></h1>

                    <p class="uk-article-meta">Oleh {{ $blogdetail->created_by }} pada {{$blogdetail->created_at->format('d-m-Y')}} dilihat {{ visits($blogdetail)->count() }} kali </p>

                    <span class="uk-text-lead uk-text-light">
                        {!! $blogdetail->deskripsi_blog !!}
                    </span>
                    
                    <div class="uk-grid-small uk-grid uk-margin-large" uk-grid="">
                        <div class="uk-first-column">
                            {{-- <a class="uk-button uk-button-text" href="#">Share Button</a> --}}
                            <a href="https://api.whatsapp.com/send?text=Bagikan%20Link%20Ini {{$blogdetail->slug_blog}} {{url('blog/read', $blogdetail->slug_blog)}}" title="Share on Whatsapp" rel="external"  target="_blank" class="uk-icon-link uk-margin-small-top" uk-icon="icon: whatsapp; ratio: 1.0"></a>
                            <a href="https://www.facebook.com/share.php?u={{url('blog/read', $blogdetail->slug_blog)}}" target="_blank" title="Share on Facebook" rel="external" class="uk-icon-link uk-margin-small-top" uk-icon="icon: facebook; ratio: 1.0"></a>
                            <a href="mailto:?Subject={{$blogdetail->judul_blog}}&Body={{$blogdetail->judul_blog}}! {{url('blog/read', $blogdetail->slug_blog)}}" class="uk-icon-link uk-margin-small-top" uk-icon="icon: mail; ratio: 1.0"></a>
                        </div>
                    </div>
